(function($) {
	...
	$.fn.Flowdesign = function(options) {
		...
		// modify by zhangfj 2016-7-18 限制拖拽父容器
		jsPlumb.draggable(jsPlumb.getSelector(".process-step"), {
			containment : $("#flowdesign_canvas")
		});
		initEndPoints();
		...
		var Flowdesign = {
			...
			getStepTempId : function() {
				var tempId = 0;
				_canvas.find("div.process-step").each(function(i) { // 生成Json字符串，发送到服务器解析
					var process_id = $(this).attr('process_id');
					if (process_id) {
						if (tempId < process_id)
							tempId = process_id;
					}
				});
				return tempId = parseInt(tempId) + 1;
			},
			getProcessInfo : function() {
				try {
					/* 连接关系 */
					var aProcessData = {};
					$("#leipi_process_info input[type=hidden]").each(
							function(i) {
								...
								if (processVal.length == 2) {
									...
									aProcessData[processVal[0]]["process_to"]
											.push(processVal[1]);
								}
							});
					/* 位置 */
					_canvas.find("div.process-step").each(function(i) { // 生成Json字符串，发送到服务器解析
						if ($(this).attr('id')) {
							...
							var cssText = $(this)[0].style.cssText;// add by
							// zhangfj
							// 2016-7-20
							if (!aProcessData[pId]) {
								aProcessData[pId] = {
							...
							aProcessData[pId]["style"] = cssText;
						}
					});
					var ret = JSON.stringify(aProcessData);
					if (ret == "{}")
						return;
					return ret;
				} catch (e) {
