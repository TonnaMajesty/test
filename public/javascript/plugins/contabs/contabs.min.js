$(function() {
	...
	function n() {
		...
		if ($(".J_menuTab").each(function() {
			return $(this).data("id") == a ? ($(this).hasClass("active") || ($(this).addClass("active").siblings(".J_menuTab").removeClass("active"), e(this), $(".J_mainContent .J_iframe").each(function() {
				return $(this).data("id") == a ? ($(this).show().siblings(".J_iframe").hide(), !1) : void 0
			})), n = !1, !1) : void 0
		}), n) {
			var s = '<a href="javascript:;" class="active J_menuTab" data-id="' + a + '">' + i + ' <i class="fa fa-times-circle"></i></a>';
			$(".J_menuTab").removeClass("active");
			var iframeId="iframe" + a;
			var r = '<iframe scrolling="no" class="J_iframe minHeight" id="' + iframeId + '" name="' + iframeId + '" width="100%" height="100%" src="' 
			+ t + '" frameborder="0" data-id="' + a + '" seamless></iframe>';
			...
			// $("body,html").animate({scrollTop:0},200); //add by zhangfj
			// 2016-10-31 点击菜单 页面回到顶端
		}
		return !1
	}
	function changeIframe(obj){
		var body=obj.contents().find("body");
		if(body==undefined)
			return;
		var oldHeight=body.height();
		var leftHeight=$("#leftNavbar").height()-30;
		obj.height(leftHeight);
		var dcount=1;
		var timer = setInterval(function(){
			if(dcount>20){// 20*300 即6s后若还没改变 则退出
				clearInterval(timer);
			}
			var height=body[0].scrollHeight;
			if((height-20)>oldHeight){
				if(height>leftHeight){
					obj.height(height);
					$("#leftNavbar").css("min-height",height+30);
				}
				clearInterval(timer);
			}
			dcount++;
	...
	/*
	 * $(".J_menuItem").each(function(t) { $(this).attr("data-index") ||
	 * $(this).attr("data-index", t) }), $(".J_menuItem").on("click", n),
	 * $(".J_menuTabs").on("click", ".J_menuTab i", s),
	 * $(".J_tabCloseOther").on("click", r), $(".J_tabShowActive").on("click",
	 * o), $(".J_menuTabs").on("click", ".J_menuTab", d),
	 * $(".J_menuTabs").on("dblclick", ".J_menuTab", c),
	 ...
	 * $(this).addClass("active") }), $(".page-tabs-content").css("margin-left",
	 * "0") })
	 */
});