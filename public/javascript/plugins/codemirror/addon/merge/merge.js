...
(function(mod) {
	if (typeof exports == "object" && typeof module == "object") // CommonJS
		mod(require("../../lib/codemirror")); // Note non-packaged dependency
	// diff_match_patch
		...
		(function(CodeMirror) {
			...
			function registerUpdate(dv) {
				...
				function update(mode) {
					...
					if (dv.showDifferences) {
						updateMarks(dv.edit, dv.diff, edit, DIFF_INSERT,
								dv.classes);
						updateMarks(dv.orig, dv.diff, orig, DIFF_DELETE,
								dv.classes);
					}
					makeConnections(dv);
				...
				function set(fast) {
					...
					if (fast === true)
						updatingFast = true;
					debounceChange = setTimeout(update, fast === true ? 20
							: 250);
				}
			...
			function syncScroll(dv, type) {
				// Change handler will do a refresh after a timeout when diff is
				// out of date
				if (dv.diffOutOfDate)
					return false;
				if (!dv.lockScroll)
				...
				if (dv.mv.options.connect == "align") {
					...
					var botDist, mix;
					// Some careful tweaking to make sure no space is left out
					// of view
					// when scrolling to top or bottom.
					if (targetPos > sInfo.top
							&& (mix = sInfo.top / halfScreen) < 1) {
						targetPos = targetPos * mix + sInfo.top * (1 - mix);
					} else if ((botDist = sInfo.height - sInfo.clientHeight
							- sInfo.top) < halfScreen) {
						var otherInfo = other.getScrollInfo();
						var botDistOther = otherInfo.height
								- otherInfo.clientHeight - targetPos;
						if (botDistOther > botDist
								&& (mix = botDist / halfScreen) < 1)
							targetPos = targetPos
									* mix
									+ (otherInfo.height
											- otherInfo.clientHeight - botDist)
									* (1 - mix);
					}
				}
			...
			function clearMarks(editor, arr, classes) {
				for ( var i = 0; i < arr.length; ++i) {
					...
					if (mark instanceof CodeMirror.TextMarker) {
						...
						editor.removeLineClass(mark, "background",
								classes.chunk);
						editor.removeLineClass(mark, "background",
								classes.start);
			...
			// FIXME maybe add a margin around viewport to prevent too many
			// updates
				...
				editor.operation(function() {
					if (state.from == state.to || vp.from - state.to > 20
						...
						if (vp.from < state.from) {
							markChanges(editor, diff, type, state.marked,
									vp.from, state.from, classes);
							state.from = vp.from;
						}
						if (vp.to > state.to) {
							markChanges(editor, diff, type, state.marked,
									state.to, vp.to, classes);
							state.to = vp.to;
						}
					}
				});
			...
			function markChanges(editor, diff, type, marks, from, to, classes) {
				...
				function markChunk(start, end) {
					...
					for ( var i = bfrom; i < bto; ++i) {
						var line = editor.addLineClass(i, "background",
								classes.chunk);
						if (i == start)
							editor.addLineClass(line, "background",
									classes.start);
						if (i == end - 1)
							editor
									.addLineClass(line, "background",
											classes.end);
						marks.push(line);
					}
					// When the chunk is empty, make sure a horizontal line
					// shows up
					if (start == end && bfrom == end && bto == end) {
						if (bfrom)
							marks.push(editor.addLineClass(bfrom - 1,
									"background", classes.end));
						else
							marks.push(editor.addLineClass(bfrom, "background",
									classes.start));
					}
				}
				...
				for ( var i = 0; i < diff.length; ++i) {
					...
					if (tp == DIFF_EQUAL) {
						var cleanFrom = pos.line
								+ (startOfLineClean(diff, i) ? 0 : 1);
						moveOver(pos, str);
						var cleanTo = pos.line
								+ (endOfLineClean(diff, i) ? 1 : 0);
			...
			function makeConnections(dv) {
				...
				var vpEdit = dv.edit.getViewport(), vpOrig = dv.orig
						.getViewport();
			...
			function findAlignedLines(dv, other) {
				...
				for ( var i = 0; i < dv.chunks.length; i++) {
					...
					linesToAlign.push([
							chunk.origTo,
							chunk.editTo,
							other ? getMatchingOrigLine(chunk.editTo,
									other.chunks) : null ]);
				}
				if (other) {
					for ( var i = 0; i < other.chunks.length; i++) {
						...
						if (j > -1)
							linesToAlign.splice(j - 1, 0,
									[
											getMatchingOrigLine(chunk.editTo,
													dv.chunks), chunk.editTo,
											chunk.origTo ]);
					}
				}
				return linesToAlign;
			...
			function drawConnectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
				...
				var top = dv.orig.heightAtLine(chunk.origFrom, "local")
						- sTopOrig;
				if (dv.svg) {
					var topLpx = top;
				...
				if (dv.copyButtons) {
					...
					copy.title = editOriginals ? "Push to left"
							: "Revert chunk";
					copy.chunk = chunk;
					copy.style.top = top + "px";

					if (editOriginals) {
						var topReverse = dv.orig.heightAtLine(chunk.editFrom,
								"local")
								- sTopEdit;
						...
						dv.type == "right" ? copyReverse.style.left = "2px"
			...
			function copyChunk(dv, to, from, chunk) {
				if (dv.diffOutOfDate)
					return;
				var editStart = chunk.editTo > to.lastLine() ? Pos(chunk.editFrom - 1)
						: Pos(chunk.editFrom, 0)
				var origStart = chunk.origTo > from.lastLine() ? Pos(chunk.origFrom - 1)
						: Pos(chunk.origFrom, 0)
				to.replaceRange(from.getRange(origStart, Pos(chunk.origTo, 0)),
						editStart, Pos(chunk.editTo, 0))
			}
			...
			var MergeView = CodeMirror.MergeView = function(node, options) {
				...
				var mergeHtml = options.mergeHtml;
				var origLeft = options.origLeft, origRight = options.origRight == null ? options.orig
						: options.origRight;
				...
				if (hasLeft) {
					...
					var leftPane = elt("div", mergeHtml.left,
							"CodeMirror-merge-pane");
					wrap.push(leftPane);
					wrap.push(buildGap(left));
				}

				var editPane = elt("div", mergeHtml.defaults,
						"CodeMirror-merge-pane");
				wrap.push(editPane);
				...
				if (hasRight) {
					...
					var rightPane = elt("div", mergeHtml.right,
							"CodeMirror-merge-pane");
					wrap.push(rightPane);
				}
				...
				if (options.collapseIdentical)
					this.editor().operation(
							function() {
								collapseIdenticalStretches(self,
				...
				var resizeInterval = setInterval(
						function() {
							for ( var p = wrapElt.parentNode; p
									&& p != document.body; p = p.parentNode) {
							}
							if (!p) {
			...
			function getDiff(a, b) {
				...
				// The library sometimes leaves in empty parts, which confuse
				// the algorithm
				for ( var i = 0; i < diff.length; ++i) {
					var part = diff[i];
					if (!part[1]) {
			...
			function goNearbyDiff(cm, dir) {
				var found = null, views = cm.state.diffViews, line = cm
						.getCursor().line;
				if (views)
					for ( var i = 0; i < views.length; i++) {
						var dv = views[i], isOrig = cm == dv.orig;
						ensureDiff(dv);
						var pos = dir < 0 ? findPrevDiff(dv.chunks, line,
								isOrig) : findNextDiff(dv.chunks, line, isOrig);
						if (pos != null
								&& (found == null || (dir < 0 ? pos > found
										: pos < found)))
							found = pos;
					}
				if (found != null)
