...
(function($) {
	...
	_bindEvent = function(setting) {
		...
		o.bind(c.NODECREATED, function(event, treeId, node) {
			tools
					.apply(setting.callback.onNodeCreated, [ event, treeId,
							node ]);
		});

		o.bind(c.CLICK, function(event, srcEvent, treeId, node, clickFlag) {
			tools.apply(setting.callback.onClick, [ srcEvent, treeId, node,
					clickFlag ]);
		});

		o.bind(c.EXPAND, function(event, treeId, node) {
			/*
			 * $("#" + treeId + " ul").each(function() { if
			 * (this.style.overflow) { this.style.overflow = ""; } });
			 */
		...
		o.bind(c.ASYNC_SUCCESS, function(event, treeId, node, msg) {
			tools.apply(setting.callback.onAsyncSuccess, [ event, treeId, node,
					msg ]);
		});

		o.bind(c.ASYNC_ERROR, function(event, treeId, node, XMLHttpRequest,
				textStatus, errorThrown) {
			tools.apply(setting.callback.onAsyncError, [ event, treeId, node,
		...
		var o = setting.treeObj, c = consts.event;
		o.unbind(c.NODECREATED).unbind(c.CLICK).unbind(c.EXPAND).unbind(
				c.COLLAPSE).unbind(c.ASYNC_SUCCESS).unbind(c.ASYNC_ERROR)
				.unbind(c.REMOVE).unbind(c.SELECTED).unbind(c.UNSELECTED);
	},
				...
				if (!node.isParent) {
					nodeEventType = "";
						...
						|| (tools.eqs(event.type, "dblclick") && tools.apply(
								setting.view.dblClickExpand, [ setting.treeId,
										node ], setting.view.dblClickExpand))) {
	...
	_initNode = function(setting, level, n, parentNode, isFirstNode,
		...
		if (n[childKey] && n[childKey].length > 0) {
			...
			n.isParent = (typeof n.isParent == "string") ? tools.eqs(
					n.isParent, "true") : !!n.isParent;
	...
	data = {
		...
		fixPIdKeyValue : function(setting, node) {
			if (setting.data.simpleData.enable) {
				node[setting.data.simpleData.pIdKey] = node.parentTId ? node
	...
	event = {
		...
		unbindTree : function(setting) {
			...
			o.unbind('selectstart', handler.onSelectStart).unbind('click',
					event.proxy).unbind('dblclick', event.proxy).unbind(
					'mouseover', event.proxy).unbind('mouseout', event.proxy)
					.unbind('mousedown', event.proxy).unbind('mouseup',
							event.proxy).unbind('contextmenu', event.proxy);
		},
		doProxy : function(e) {
			var results = [];
	...
	handler = {
		...
		onClickNode : function(event, node) {
			var setting = data.getSetting(event.data.treeId), clickFlag = ((setting.view.autoCancelSelected && (event.ctrlKey || event.metaKey)) && data
					.isSelectedNode(setting, node)) ? 0
					: (setting.view.autoCancelSelected
							&& (event.ctrlKey || event.metaKey) && setting.view.selectedMulti) ? 2
							: 1;
			...
			setting.treeObj.trigger(consts.event.CLICK, [ event,
		...
		onZTreeMouseup : function(event, node) {
			...
			if (tools.apply(setting.callback.beforeMouseUp, [ setting.treeId,
					node ], true)) {
				tools.apply(setting.callback.onMouseUp, [ event,
						setting.treeId, node ]);
			}
			return true;
		},
	...
	tools = {
		...
		clone : function(obj) {
			...
			for ( var i in obj) {
				o[i] = (obj[i] instanceof Date) ? new Date(obj[i].getTime())
						: (typeof obj[i] === "object" ? tools.clone(obj[i])
								: obj[i]);
			}
			return o;
		},
		...
		$ : function(node, exp, setting) {
			...
			if (typeof node == "string") {
				return $(node, setting ? setting.treeObj.get(0).ownerDocument
						: null);
			} else {
	...
	view = {
		addNodes : function(setting, parentNode, index, newNodes, isSilent) {
			...
			if (parentNode) {
				...
				data.addNodesData(setting, data.getRoot(setting), index,
						newNodes);
			...
			for ( var i = 0, l = nodes.length; i < l; i++) {
				...
				if (node[childKey] && node[childKey].length > 0) {
					...
					childHtml = view.appendNodes(setting, level + 1,
							node[childKey], node, -1, initFlag, openFlag
									&& node.open);
				}
				if (openFlag) {
			...
			$.ajax({
				...
				success : function(msg) {
					...
					if (newNodes && newNodes !== "") {
						...
						view.addNodes(setting, node, -1, !!newNodes ? tools
		...
		cancelPreSelectedNode : function(setting, node, excludeNode) {
			...
			for (i = list.length - 1; i >= 0; i--) {
				n = list[i];
				if (node === n
						|| (!node && (!excludeNode || excludeNode !== n))) {
					$$(n, consts.id.A, setting).removeClass(
		...
		createNodes : function(setting, level, nodes, parentNode, index) {
			...
			var zTreeHtml = view.appendNodes(setting, level, nodes, parentNode,
					index, true, openFlag), parentObj, nextObj;
		...
		expandCollapseNode : function(setting, node, expandFlag, animateFlag,
			...
			if (node.isParent) {
				...
				if (node.open) {
					...
					ulObj.css("overflow", "");// add by zhangfj 2016-11-15
				} else {
		...
		expandCollapseSonNode : function(setting, node, expandFlag,
				animateFlag, callback) {
			var root = data.getRoot(setting), childKey = setting.data.key.children, treeNodes = (node) ? node[childKey]
					: root[childKey], selfAnimateSign = (node) ? false
		...
		makeDOMNodeIcon : function(html, setting, node) {
			var nameStr = data.getNodeName(setting, node), name = setting.view.nameIsHTML ? nameStr
					: nameStr.replace(/&/g, '&amp;').replace(/</g, '&lt;')
							.replace(/>/g, '&gt;');
			// modify by zhangfj 新增isparent属性
			var spanId = node.tId + consts.id.SPAN;
			html.push("<span id='", node.tId, consts.id.ICON,
					"' title='' treeNode", consts.id.ICON, " class='", view
							.makeNodeIcoClass(setting, node), "' style='", view
							.makeNodeIcoStyle(setting, node),
					"'></span><span isparent='", node.isParent, "' id='",
					spanId, "' class='", consts.className.NAME, "'>", name,
					"</span>");
		},
		...
		makeNodeIcoClass : function(setting, node) {
			...
			if (!node.isAjaxing) {
				icoCss[0] = (node.iconSkin ? node.iconSkin + "_" : "")
						+ icoCss[0];
				if (node.isParent) {
					icoCss.push(node.open ? consts.folder.OPEN
							: consts.folder.CLOSE);
				} else {
		...
		makeNodeIcoStyle : function(setting, node) {
			...
			if (!node.isAjaxing) {
				...
				if (setting.view.showIcon == false
						|| !tools.apply(setting.view.showIcon, [
								setting.treeId, node ], true)) {
		...
		makeNodeLineClass : function(setting, node) {
			...
			if (node.isParent) {
				lineClass.push(node.open ? consts.folder.OPEN
						: consts.folder.CLOSE);
			} else {
		...
		removeNode : function(setting, node) {
			var root = data.getRoot(setting), childKey = setting.data.key.children, parentNode = (node.parentTId) ? node
					.getParentNode()
					: root;
		...
		selectNode : function(setting, node, addFlag) {
			...
			setting.treeObj.trigger(consts.event.SELECTED, [ setting.treeId,
					node ]);
		},
		...
		init : function(obj, zSetting, zNodes) {
			...
			var zTreeTools = {
				...
				addNodes : function(parentNode, index, newNodes, isSilent) {
					...
					var xNewNodes = tools
							.clone(tools.isArray(newNodes) ? newNodes
									: [ newNodes ]);
				...
				changezTreeStyle : function(tId, oriWidth) {
					/*
					 * var maxWidth = treeWidth = $("#" + tId).width();
					 * setTimeout(function() { $(".node_name").each( function() {
					 * var width = $(this)[0].offsetLeft +
					 * $(this)[0].offsetWidth; if (oriWidth) {// 折叠on if (width >
					 * oriWidth) maxWidth = width; } else { if (width >
					 * maxWidth) maxWidth = width; } }); if (oriWidth &&
					 * maxWidth == treeWidth) { maxWidth = oriWidth; } if
					 * (maxWidth != treeWidth) { $("#" + tId).width(maxWidth); } },
					 * 1);
					 */
				},
				...
				getNodeByParam : function(key, value, parentNode) {
					...
					return data.getNodeByParam(setting,
							parentNode ? parentNode[setting.data.key.children]
				...
				getNodeIndex : function(node) {
					if (!node)
						return null;
					var childKey = setting.data.key.children, parentNode = (node.parentTId) ? node
							.getParentNode()
				...
				reAsyncChildNodes : function(parentNode, reloadType, isSilent) {
					...
					if (reloadType == "refresh") {
						...
						for ( var i = 0, l = parentNode[childKey] ? parentNode[childKey].length
								: 0; i < l; i++) {
				...
				refresh : function() {
					...
					view.createNodes(setting, 0,
							root[setting.data.key.children], null, -1);
				},
