...
class AuthMenuService1 {
	...
	public function getAllMenusJson($user) {
		try {
			...
			if ($user->intAdmin)
				...
				sidm.chrDisplayBGPath AS menuImg,smu.intIsDisplayDesktop,smu.intIsDisplayLeft,smenu.intParentID
				from sys_menus smenu
				LEFT JOIN sys_menu_users smu on smu.intMenuID=smenu.id
				LEFT JOIN users u on u.id=smu.intUserID and u.intAdmin=1
				LEFT JOIN sys_img_details sidm on sidm.id=smenu.intImgDetailID";
			$res = DB::select ( "select smc.id as classifyId,smc.chrClassifyName,smc.intClassifyAsc,simg.chrDisplayBGPath AS classifyImg,
				smenu.id,smenu.chrMenuName,smenu.chrMenuMethodArgs,smenu.intMenuAsc,smenu.menuImg,
				smenu.intIsDisplayDesktop,smenu.intIsDisplayLeft,smenu.intParentID
				...
				sidm.chrDisplayBGPath AS menuImg,smu.intIsDisplayDesktop,smu.intIsDisplayLeft,smenu.intParentID
				from sys_menus smenu
				INNER JOIN sys_priviliges spu on spu.intPriviligeOperationTypeValue=smenu.id and spu.chrPriviligeOperationType='menu'
				LEFT JOIN sys_img_details sidm on sidm.id=smenu.intImgDetailID
				LEFT JOIN sys_menu_users smu on smu.intMenuID=spu.intPriviligeOperationTypeValue
				where  spu.intPriviligeTypeValue=$user->id and spu.chrPriviligeType='user'
				UNION
				SELECT smenu.intClassifyID,smenu.id,smenu.chrMenuName,smenu.chrMenuMethodArgs,smenu.intMenuAsc,
				sidm.chrDisplayBGPath AS menuImg,smu.intIsDisplayDesktop,smu.intIsDisplayLeft,smenu.intParentID
				from sys_menus smenu
				INNER JOIN sys_priviliges spu on spu.intPriviligeOperationTypeValue=smenu.id and spu.chrPriviligeOperationType='menu'
				INNER JOIN sys_roles sr on sr.id=spu.intPriviligeTypeValue and spu.chrPriviligeType='role'
				INNER JOIN sys_user_roles sur on sur.intRoleID=sr.id and sur.intUserID=$user->id
				LEFT JOIN sys_menu_users smu on smu.intMenuID=spu.intPriviligeOperationTypeValue
				LEFT JOIN sys_img_details sidm on sidm.id=smenu.intImgDetailID
				$adminSQL
	...
	public function insert($menu, $user) {
		DB::insert ( "insert into sys_menus (chrMenuName,chrMemo,intCreaterID,intMenuAsc,intParentID,chrMenuMethodArgs)
				values (?,?,?,?,?,?)", [ 
				$menu ['menuName'],
				$menu ['memo'],
				$user->id,
