...
class EasyCase(TestCase):
	...
	def runTest(self):
		...
		driver.find_element_by_css_selector("#root > div > div > div > div > div > div.ant-spin-container > div > div > div > span > div.ant-table-body > table > tbody > tr:nth-child(4) > td:nth-child(5) > button").click()  # 点击设置
		...
		driver.find_element_by_css_selector('body > div:nth-child(8) > div > div.ant-modal-wrap > div > div.ant-modal-content > div.ant-modal-body > div > div > div > div.ant-tabs-content > div:nth-child(1) > div > div > div.ck-customize-gongn01 > div > ul > li > div.ck-gongncnt-first > input[type="text"]').send_keys(u'所属区域超客营销客户自定义自动化')  # 输入选项信息
